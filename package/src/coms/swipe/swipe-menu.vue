<style scoped>
  ...
  .x-swipe .x-swipe-items-wrap {
    position: relative;
    overflow: hidden;
    height: 100%;
  }
  .x-swipe .x-swipe-items-wrap > div {
    position: absolute;
  ...
  .x-swipe .x-swipe-items-wrap > div.is-active {
    display: flex;
    transform: none;
  }
  .x-swipe .x-swipe-indicators {
    position: absolute;
    bottom: 10px;
    left: 50%;
  ...
  .x-swipe .x-swipe-indicator.is-active {
    opacity: 1;
    background: #9AE8E3;
  }
</style>
...
<script>
import { once, addClass, removeClass } from '../../js/dom';
